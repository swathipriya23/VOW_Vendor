...
budgetbuildersearech_click(data){
  ...
  this.dataService.getbudgetsearchsummary(this.pprSearchForm.value)
        .subscribe((results: any[]) => {
  ...
  if(data.Padding_left=='50px'){
    ...
    if(data.Padding_left=='100px'){
      for (let i = a2; i < data1.length; i++) {
        let a1=data1[i]
        a.push(i) 
        if ((a1.Padding_left=='100px')||(a1.Padding_left=='50px') || (a1.Padding_left=='75px') ||(a1.Padding_left=='10px')) { break; }
        console.log ("Block statement execution no." + i);
      }}
  a.pop()
...
// employee search start
  ...
  save_draft(data,data1,status){
    ...
    if(this.type=='Quarterly'){
      let a11=[]
        ...
        }console.log('draft_data',input_ary)
    ...
    if(this.type=='Monthly'){
      let a11=[]
        for (let i = 0; i < data.length; i++) {
          let a1=data[i]
          if((data[i].Padding_left=='120px') && (data[i].change_flag=='Y'))
          a11.push(a1) 
  ...
  private set_budgetbuiler(prokeyvalue){
    this.SpinnerService.show()
    this.dataService.set_budgetbuilder(prokeyvalue)
      .subscribe((results: any[]) => {
        this.SpinnerService.hide()
